...
$(function(){
	$('#RegistIn').click(function(){
		//javascript:alert($('#resident_0').val());
		...
		for (var i=0; i<nodes.length; i++) {
		        // 如果类型是 radio 也符合要求，而且也被选中了
		        if (nodes[i].type==='radio' && nodes[i].checked) {
		            //返回相应的值
		            resident = nodes[i].value;
		        }
		    }
		...
		$.ajax({
			...
			url:url,
			dataType: "json",
			data:{name:name,resident:resident,password1:password1,password2:password2},
...
<div class="login shadow" style="padding-bottom: 20px;">
                ...
                <div class="member">
                    <span>已有MICT帐号？</span> <a href="<?php echo Yii::app()->createUrl('site/login')?>">请登录</a>
                </div>
</div>